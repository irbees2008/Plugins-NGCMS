...
function generateSitemap(){
	global $template, $tpl, $lang, $mysql, $config, $parse, $catz, $SYSTEM_FLAGS, $TemplateCache;
	...
	foreach ($news as $row)	{
		...
		if ($cu_c <> $row['name']) {	
			...
			$tvars['vars']['cat_title'] = $goright.GetCategories($row['catid']).$link_rss;
			$cu_c = $row['name'];
		}
		else
			$tvars['vars']['cat_title'] =  '';
				
					
		$tvars['vars']['title'] = $row['title'];
		$tvars['vars']['date'] = langdate($config['timestamp_active'], $row['postdate']);
		$tvars['vars']['views'] = $row['views'];
		$tvars['vars']['comm-num'] = $row['com'];
		$tvars['vars']['[link]']	='<a href="'.newsGenerateLink(array('catid' => $row['catid'], 'alt_name' => $row['alt_name'], 'id' => $row['id'], 'postdate' => $row['postdate']), false, 0, true).'">';
		$tvars['vars']['[/link]'] = '</a>';
		
		$tpl -> template('sitemap_news', $tpath['sitemap_news']);
		$tpl -> vars('sitemap_news', $tvars);
		
		$sitemap_out .= $tpl -> show('sitemap_news');
	}
	...
	if ($pages_count > 1){
		...
		$tvars['vars']['pagination'] = generatePagination($page, 1, $pages_count, 9, $paginationParams, $navigations);
		# set plugin title
	...
	$tvars['vars']['sitemap_news'] = $sitemap_out;
	
	$tpl -> template('sitemap', $tpath['sitemap']);
	$tpl -> vars('sitemap', $tvars);
	
	$result = 	$tpl -> show('sitemap');
