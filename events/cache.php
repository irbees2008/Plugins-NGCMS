...
{global $mysql, $config;
	...
	if(!file_exists($events_dir.'/cache_entries_cnt.php') or $load){
		$result = $mysql->result('SELECT COUNT(id) FROM '.prefix.'_events WHERE expired != \'1\' ');
		file_put_contents($events_dir.'/cache_entries_cnt.php', serialize($result));
	}
...
{global $mysql, $config;
	...
	if(!file_exists($events_dir.'/cache_catz.php') or $load){
		
		$catt = array(); 
		foreach ($mysql->select('SELECT cat_id, COUNT(id) as num FROM '.prefix.'_events WHERE expired != \'1\' GROUP BY cat_id ') as $rows)
		{
		...
		file_put_contents($events_dir.'/cache_catz.php', serialize($catz_tree));
	}
