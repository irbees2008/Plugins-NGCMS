...
class NeighboringNewsFilter extends NewsFilter {
	...
	$tpath = locatePluginTemplates(array('neighboring_news', 'next_news', 'previous_news'), 'neighboring_news', extra_get_param('neighboring_news', 'localsource'));
	...
	if (extra_get_param('neighboring_news','full_mode') && $mode['style'] == 'full'){
		...
		if(intval($SQLnews['catid'])!=null){
			...
			if (intval(extra_get_param('neighboring_news','compare')) == 1)	$id = $fcat; 
