...
{global $tpl, $mysql;
	...
	if (isset($_REQUEST['submit']))
	{
		if(isset($_REQUEST['url']) && !empty($_REQUEST['url']))
		{
			...
			$ULIB->registerCommand('zboard', 'show',
				array ('vars' =>
			...
			$ULIB->registerCommand('zboard', 'send',
				array ('vars' =>
						array(),
			...
			$ULIB->registerCommand('zboard', 'search',
				array ('vars' =>
						array(),
			...
			$ULIB->registerCommand('zboard', 'list',
				array ('vars' =>
			...
			$ULIB->registerCommand('zboard', 'edit',
				array ('vars' =>
						array(	'id' => array('matchRegex' => '\d+', 'descr' => array('russian' => 'ID объявления')),
						),
			...
			$ULIB->registerCommand('zboard', 'del',
				array ('vars' =>
						array(	'id' => array('matchRegex' => '\d+', 'descr' => array('russian' => 'ID объявления')),
						),
			...
			$ULIB->registerCommand('zboard', 'expend',
				array ('vars' =>
						array(	'id' => array('matchRegex' => '\d+', 'descr' => array('russian' => 'ID объявления')),
		...
		*/
		...
		$pvars['vars'] = array (
			'images_url' => str_replace( array( '{url_images}', '{url_images_thumb}'), 
							array(images_url.'/zboard/'.$row['plugin_images'], images_url.'/zboard/thumb/'.$row['plugin_images']), $lang['zboard']['images_url']),
			'options' => $options,
...
global $tpl, $mysql, $cron;
	...
	if (isset($_REQUEST['submit']))
	{
		...
		if( ($value['plugin']=='zboard') && ($value['handler']=='zboard_views') ) {  $cron_min = $value['min']; $cron_hour = $value['hour']; $cron_day = $value['day']; $cron_month = $value['month']; }
		}
		...
		if( ($value_1['plugin']=='zboard') && ($value_1['handler']=='zboard_expired') ) {  $cron_min = $value_1['min']; $cron_hour = $value_1['hour']; $cron_day = $value_1['day']; $cron_month = $value_1['month']; }
		}
		if(!isset($cron_min)) { $cron_min = '0,15,30,45'; }
		if(!isset($cron_hour)) { $cron_hour = '*'; } 
		if(!isset($cron_day)) { $cron_day = '*'; } 
