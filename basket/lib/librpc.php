...
function basket_add_item($linked_ds, $linked_id, $title, $price, $count, $xfld = array()) {
	...
	return array('status' => 1, 'errorCode' => 0, 'data' => ' Добавить в корзину ', 'update' => arrayCharsetConvert(0, $xt->render($tVars)));
}
	...
	switch ($params['action']) {
		...
		case 'add':
			...
			if (!(in_array($linked_ds, array($DSlist['news'], $DSlist['#xfields:tdata'])))) {
				return array('status' => 0, 'errorCode' => 2, 'errorText' => 'Корзина может быть использована только для Товара');
			}

			// Check available DataSources
			if ($count < 1) {
				return array('status' => 0, 'errorCode' => 2, 'errorText' => 'Стоймость должна быть положительной');
			}
