...
function comments_show($newsID, $commID = 0, $commDisplayNum = 0, $callingParams = array()){
	...
	if (!isset($callingParams['module']))
	{
		$callingParams['module'] = 'news';
	}

	$where = 'c.post='.db_squote($newsID);
	if (isset($callingParams['gallery']))
	{
		$news_ids = array();
		foreach ($mysql->select("select id from ".prefix."_images where folder in (select name from ".prefix."_gmanager where id=".$callingParams['gallery'] . ")") as $irow)
		{
			$news_ids[] = $irow[0];
		}
		$where = 'c.post IN ('.implode(',', $news_ids).')';
	}
	...
	if (isset($joinFilter['users']) && isset($joinFilter['users']['fields']) && is_array($joinFilter['users']['fields']) && (count($joinFilter['users']['fields']) > 0)) {
		$sql = "select c.*, ".
			...
			' left join '.uprefix.'_users u on c.author_id = u.id where module='.db_squote($callingParams['module']).' and '.$where.($commID?(" and c.id=".db_squote($commID)):'');
	} else {
		$sql = "select c.* from ".prefix."_comments c WHERE ".$where.($commID?(" and c.id=".db_squote($commID)):'');
	}
	...
	foreach ($mysql->select($sql) as $row) {
		...
		if (isset($callingParams['gallery']))
		{
			...
			$irow = $mysql->record("select * from ".prefix."_images where id=".$row['post']);
			
			$fileurl	=	$fmanager->uname.'/'.$irow['folder'].'/'.$irow['name'];
			$thumburl	=	file_exists($fmanager->dname.$irow['folder'].'/thumb/'.$irow['name'])?$fmanager->uname.'/'.$irow['folder'].'/'.'thumb/'.$irow['name']:$fileurl;

			$tvars['vars']['url_entry'] = generatePluginLink('gmanager', 'image', array(
					'gallery' => $irow['folder'],
					'id' => $irow['id'],
					'name' => $irow['name']
				)
			...
			$tvars['regx']["'\[gallery\](.*?)\[/gallery]'si"] = '$1';
		}
		else
		{
			$tvars['vars']['url_entry'] = '';
			$tvars['vars']['url_image'] = '';
			$tvars['vars']['url_image_thumb'] = '';
			$tvars['regx']["'\[gallery\](.*?)\[/gallery]'si"] = '';
		}
...
function comments_showform($newsID, $callingParams = array()){
	...
	$tvars['vars']['module']	=	$callingParams['module'] ? $callingParams['module'] : 'news';
