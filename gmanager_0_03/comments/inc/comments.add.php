...
//
function comments_add($callingParams = array()){
	...
	$SQL['module'] = $callingParams['module'];

	// Locate news
	if ($callingParams['module'] == 'news' && ($news_row = $mysql->record("select * from ".prefix."_news where id = ".db_squote($SQL['post'])))) {
	...
	} else if ($news_row = $mysql->record("select * from ".prefix."_".$callingParams['module']." where id = ".db_squote($SQL['post']))) {
	} else {
		msg(array("type" => "error", "text" => $lang['comments:err.notfound']));
		return;
	}
	...
	$mysql->query("update ".prefix."_".$callingParams['module']." set com=com+1 where id=".db_squote($SQL['post']));
